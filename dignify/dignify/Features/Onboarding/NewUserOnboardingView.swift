...
///
/// 풀을 못 받으면(네트워크 실패·서버 미시딩) 고르기를 건너뛰고 바로 완료로 간다.
/// 온보딩이 막히는 것보다 시드 없이 시작하는 편이 낫다 — 그 유저는 콜드스타트 피드가 받는다.
struct NewUserOnboardingView: View {
    @Environment(AppSession.self) private var appSession
    @State private var pool: [API.FeedItem] = []
    @State private var isLoading = true
    @State private var showPicker = false
    /// 튜토리얼이 끝났는데 풀이 아직 안 온 상태. 이때는 화면을 튜토리얼 마지막 장에 두고
    /// 응답을 기다린다 — 빈 그리드를 잠깐 보여주는 것보다 낫다.
    @State private var waitingForPool = false

    var body: some View {
        Group {
            if showPicker {
                SeedPoolPickerView(pool: pool) { _ in try await complete() }
            } else {
                // 피드 조작을 먼저 익히게 한다. 풀은 이 동안 백그라운드로 받는다.
                TutorialView { finishTutorial() }
            }
        }
        .background(DSColor.background)
        .task {
            pool = await SeedPoolPickerView.fetch(appSession)
            isLoading = false
            // 튜토리얼을 먼저 끝냈으면(= 기다리는 중) 여기서 갈린다.
            if waitingForPool { enterPicker() }
        }
    }

    private func finishTutorial() {
        if isLoading { waitingForPool = true; return }
        enterPicker()
    }

    /// 풀이 비어도 그대로 넘긴다 — 화면이 빈 그리드와 건너뛰기 버튼을 준다.
    /// 여기서 조용히 완료 처리를 해버리면 그 요청이 실패했을 때 다시 시도할 자리가 없다.
    private func enterPicker() {
        waitingForPool = false
        showPicker = true
    }
